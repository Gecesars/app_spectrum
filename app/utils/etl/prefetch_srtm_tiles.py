...
def _collect_coords(table: str) -> Iterable[Tuple[float, float]]:
    sql = sa.text(f"SELECT ST_Y(geom) AS lat, ST_X(geom) AS lon FROM {table} WHERE geom IS NOT NULL")
    for row in db.session.execute(sql):
        yield float(row.lat), float(row.lon)


def prefetch(load: bool = False) -> Set[str]:
    tiles: Set[str] = set()
    # Coleta FM e TV (radcom não é prioridade para terrenos altos)
    for lat, lon in _collect_coords("estacoes_fm"):
        tiles.add(tile_name(lat, lon))
    for lat, lon in _collect_coords("estacoes_tv"):
        tiles.add(tile_name(lat, lon))
